...
class DecsFlowException(ViolinException):
    def __init__(self, not_matched_data_keys, data_keys, decs, flow_stage=None, transform=None):
        self.not_matched_data_keys, self.data_keys, self.decs, self.flow_stage, self.transform = not_matched_data_keys, data_keys, decs, flow_stage, transform
    def __str__(self):
        return f"""Decs flow error at transform {self.transform} stage {self.flow_stage}.
Decs {str(self.decs)} didn't match with data_keys: {str(self.data_keys)}.
